<template>
  <div class='flex flex-row'>
    <DataColumn class='shrink-0' title='Programme' @open='dataColumnOpened = true' @close='dataColumnOpened = false'>
      ...
      <template v-if='programLoaded'>
        ...
        <template v-for='(pair) in editableLevelsPairs' :key='pair.level.id'>
          <div class='h-min'>
            <div class='flex justify-between text-gray-500 hover:text-gray-700'>
              <SmallTitle class='flex-1 transition-colors cursor-default'>
                {{ pair.level.fullName }}
              </SmallTitle>
              <ActionIcon
                icon='expand_more'
            ...
            <div
              class='transition-all overflow-y-clip'
              :class='{
            ...
            >
              <template v-if='pair.themes.length > 0'>
                <template v-for='theme in pair.themes' :key='theme.uuid'>
                  <div
                    class='font-semibold text-gray-500 p-2 my-1 rounded-lg cursor-pointer flex'
                    :class='{
                  ...
                  >
                    ...
                    <Icon :icon='theme.visible ? "visibility" : "visibility_off"' class='ml-2 text-gray-500'/>
                  </div>
                </template>
              </template>
              <template v-else>
                <div class='p-2 my-1 rounded-lg'>
                  <p class='text-gray-500 text-sm italic whitespace-normal'>Il n'y a aucun thème dans ce niveau pour le moment.</p>
                </div>
              </template>
            </div>
          </div>
        ...
        <ActionButton primary v-if='teachingLevels.length' class='w-full mt-8' @click='addNewTheme()'>
          <Icon icon='add' />
          {{ atLeastOneTheme ? 'Ajouter un nouveau thème' :'Créer votre premier thème' }}
        </ActionButton>

        <p v-else class='text-center font-semibold text-gray-400 italic text-lg mt-8'>
          Vous n'avez accès aux thèmes d'aucun niveau.
        </p>
      </template>

      <template v-else>
        <SkeletonText v-for='index in 7' :key='index' class='w-full my-4' xl />
      </template>
    </DataColumn>

    <div class='flex-1 min-w-0' :class='{ "hidden md:block": dataColumnOpened}'>
      <template v-if='!programLoaded || selectedTheme'>
        <div class='p-8 max-w-full'>
          <template v-if='programLoaded'>
            <div class='flex flex-row'>
              <div class='flex-1'>
                <LargeTitle>{{ selectedTheme.name }} - {{ selectedTheme.level.fullName }}</LargeTitle>
                <p class='font-semibold text-md'>{{ selectedTheme.description }}</p>
              </div>
              <div class='flex'>
                <ActionIcon icon='edit' class='my-4 mx-1' @click='editSelectedTheme' 2xl :disabled='selectedThemeUpdating' />
                <ActionIcon :icon='selectedTheme.visible ? "visibility" : "visibility_off"' class='my-4 mx-1' @click='toggleSelectedThemeVisibility' 2xl :disabled='selectedThemeUpdating' />
                <ActionIcon icon='delete' class='my-4 mx-1' @click='deleteSelectedTheme' 2xl :disabled='selectedThemeUpdating' />
              </div>
            </div>
          </template>
          ...
          <div class='mt-12 md:mt-28 max-w-full'>
      ...
      <template v-else>
        <div class='center h-main'>
          <p class='text-center font-semibold text-gray-400 italic text-lg m-2'>
    ...
    >
      <form @submit.prevent='submitThemeEdit' @keydown="preventEnter">
        ...
        <ActionButton cancel small class='mt-8 w-28' @click.prevent='cancelThemeEdit'>Annuler</ActionButton>
        <SubmitButton
          class='mt-8 w-28'
    ...
    >
      <form @submit.prevent='submitResourceEdit' @keydown="preventEnter">
        ...
        <div class='flex place-content-between'>
        <ActionButton cancel small class='mt-8 w-28' @click.prevent='cancelResourceEdit'>Annuler</ActionButton>
        <SubmitButton
          class='mt-8 w-28'
          :message='editThemePanelContinueMessage'
...
<script setup lang='ts'>
  ...
  import type { SchoolProgram, Theme, ThemeResource, ThemeResourceFile } from '@/database/interface/school_program'
  ...
  const editThemePanelInvalidFields = computed(() => {
    return editThemePanelOpen.value && (
  ...
  watch(selectedTheme, (newTheme) => {
    ...
    databaseClient.getThemeResources(newTheme.uuid)
      .then((resources) => {
  ...
  const editResourcePanelInvalidFields = computed(() => {
    return editResourcePanelOpen.value && (
  ...
  databaseClient.getTeachingLevels()
    .then(levels => {
      teachingLevels.value = levels.map(level => level.fullName)
    })
    .catch(MessageStack.logError)

  const atLeastOneTheme = computed(() => editableLevelsPairs.value.some(pair => pair.themes.length > 0))
  ...
  function cancelThemeEdit() {
    ...
    if (editThemeIsCreating) {
      contentChanged = editedThemeName.value.length > 0 || editedThemeDescription.value.length > 0 || editedThemeLevel.value != teachingLevels.value[0]
    } else {
      contentChanged = editedThemeName.value !== selectedTheme.value!.name || editedThemeDescription.value !== selectedTheme.value!.description || editedThemeLevel.value !== selectedTheme.value!.level!.fullName
    }

    if (contentChanged) {
      PopupManager.getInstance().confirm({
        title: 'Annuler',
        content: 'Êtes-vous sûr de vouloir annuler ? Vos modifications seront perdues.',
  ...
  function submitThemeEdit() {
    ...
    if (editThemeIsCreating) {
      ...
      submittingPromise = databaseClient.updateTheme(
  ...
  function submitResourceEdit() {
    ...
    submittingPromise
      .then(async () => {
        refreshSelectedThemeResources((await databaseClient.getThemeResources(selectedTheme.value!!.uuid))!!)
        replacer.replaceLastBy({
          type: MessageType.SUCCESS,
          text: successMessage
        })
  ...
  async function refreshProgram(program: SchoolProgram) {
    ...
    if (!selectedTheme.value) {
      const firstTheme = editableLevelsPairs.value?.find(pair => pair.themes.length > 0)?.themes[0]
      if (firstTheme) {
        selectedTheme.value = firstTheme
      }
    } else {
      selectedTheme.value = program.find((theme) => theme.uuid == selectedTheme.value!!.uuid) ?? undefined
    }
  }

  async function refreshSelectedThemeResources(resources: ThemeResource[]) {
    selectedThemeResources.value = resources.sort((a, b) => a.name.localeCompare(b.name))
    resourcesUpdating.value = resources.map(() => false)
    existingResourceTypes.value = (await databaseClient.getThemeResourceTypes()).map(type => type.name)
  }
  ...
  function editResource(index: number) {
    ...
    editedResourceCorrected.value = editedResource.correction ? CORRECTED : NOT_CORRECTED
  ...
  function toggleResource(index: number) {
    ...
    databaseClient.changeThemeResourceVisibility(selectedTheme.value!!.uuid, selectedThemeResources.value!![index].uuid, !selectedThemeResources.value!![index].visible)
      .then(async () => {
        replacer.replaceLastBy({
          type: MessageType.SUCCESS,
  ...
  function deleteResource(index: number) {
    PopupManager.getInstance().confirm({
      ...
      okCallback: () => {
        ...
        databaseClient.deleteThemeResource(selectedTheme.value!!.uuid, selectedThemeResources.value!![index].uuid)
          .then(async () => {
            replacer.replaceLastBy({
              type: MessageType.SUCCESS,
  ...
  function cancelResourceEdit() {
    let contentChanged = false
    if (editResourceIsCreating) {
      contentChanged = editedResourceName.value.length > 0 || editedResourceMessage.value.length > 0 || editedResourceType.value.length > 0 || editedResourceFiles.value.length > 0
    } else {
      contentChanged = editedResourceName.value != editedResource!!.name || editedResourceMessage.value != editedResource!!.message || editedResourceType.value != editedResource!!.type || editedResourceFiles.value != editedResource!!.content
    }
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (!/^(\w|\/|!|-|\.|\*|'|\(|\)| |&|\$|@|=|;|:|\+|,|\?)*$/.test(file.name)) {
        MessageStack.getInstance().push({
          type: MessageType.ERROR,
          text: 'Le nom du fichier "' + file.name + '" contient des caractères non autorisés et n\'a donc pas été ajouté.'
        })
      } else if (editedResourceFiles.value.find(existingFile => (existingFile as any).name === file.name)) {
        MessageStack.getInstance().push({
          type: MessageType.ERROR,
          text: 'Le fichier "' + file.name + '" existe déjà et n\'a donc pas été ajouté.'
        })
      } else {
  ...
  function addResourceURL(url: string) {
    if (editedResourceFiles.value.find(existingFile => (existingFile as any).url === url)) {
      MessageStack.getInstance().push({
        type: MessageType.ERROR,
