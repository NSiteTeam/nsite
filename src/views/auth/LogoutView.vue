<script setup lang="ts">
    ...
    databaseClient.logout().then((success: boolean) => {
        if (success) {
            ...
            message.value = 'Une erreur a été rencontrée durant la tentative de déconnexion'
...
<template>
    <div id="disconnection-message">
        <p :class="{ good: disconnectionSuccess, indication: isDisconnecting, error: encounteredError }">{{ message }}</p>
